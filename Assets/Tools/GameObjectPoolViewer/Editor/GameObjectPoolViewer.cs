...
public class GameObjectPoolViewer : EditorWindow
{
    static List<string> m_vecInfoList = new List<string>();
    Vector2 m_vScrollViewPos = Vector2.zero;

    [MenuItem("[打包工具]/Asset/Game Object Viewer")]
    static void Init()
    {
        var window = GetWindow<GameObjectPoolViewer>();
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("抓取资源池详情"))
            CGameObjectPool.instance.DumpGameObjectInfo(ref m_vecInfoList);

        GUI.color = Color.yellow;
        if (GUILayout.Button("导出资源信息到文件"))
        {
            string date = DateTime.Now.ToString("yyyy_MM_dd_hh_mm_ss");
            FileStream streamW = new FileStream("GameObjDump_" + date + ".txt", FileMode.Create, FileAccess.Write, FileShare.Write);
