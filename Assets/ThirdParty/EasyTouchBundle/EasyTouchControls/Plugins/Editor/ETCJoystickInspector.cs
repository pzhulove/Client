...
public class ETCJoystickInspector:Editor  {
	...
	public override void OnInspectorGUI(){
		...
		t.gameObject.name = EditorGUILayout.TextField("摇杆名字",t.gameObject.name);
		t.activated = ETCGuiTools.Toggle("是否激活",t.activated,true);
		t.visible = ETCGuiTools.Toggle("是否可视",t.visible,true);

		EditorGUILayout.Space();
		t.useFixedUpdate = ETCGuiTools.Toggle("使用 Fixed Updae",t.useFixedUpdate,true);
		...
		if (t.showPSInspector){
			ETCGuiTools.BeginGroup();{
				...
				// Type
				t.joystickType = (ETCJoystick.JoystickType)EditorGUILayout.EnumPopup("类型",t.joystickType);

				if (t.joystickType == ETCJoystick.JoystickType.Static){
					t.anchor = (ETCBase.RectAnchor)EditorGUILayout.EnumPopup( "锚点",t.anchor);
				...
				//	if (t.isNoOffsetThumb) t.isNoReturn = false;
					...
					if (t.joystickArea == ETCJoystick.JoystickArea.UserDefined){
						t.userArea = (RectTransform)EditorGUILayout.ObjectField("自定区域",t.userArea,typeof(RectTransform),true);
					}
                    t.maxOffsetX = EditorGUILayout.IntField("X轴范围限制", t.maxOffsetX);
                    t.maxOffsetY = EditorGUILayout.IntField("Y轴范围限制", t.maxOffsetY);
                }
				...
				// Area Size
				if (ratio>=1){
					float s = EditorGUILayout.FloatField("背景图片大小", t.rectTransform().rect.width);
					...
					float s = EditorGUILayout.FloatField("背景图片大小", t.rectTransform().rect.height);
				...
				// Thumb size
				if (ratio>=1){
					float s = EditorGUILayout.FloatField("摇杆球大小", t.thumb.rectTransform().rect.width);
					...
					float s = EditorGUILayout.FloatField("摇杆球大小", t.thumb.rectTransform().rect.height);
				...
				t.radiusBase = (ETCJoystick.RadiusBase)EditorGUILayout.EnumPopup("有效范围",t.radiusBase );
				if (t.radiusBase == ETCJoystick.RadiusBase.UserDefined){
					t.radiusBaseValue = EditorGUILayout.FloatField("自定义半径",t.radiusBaseValue);
				}
				EditorGUILayout.Space();
