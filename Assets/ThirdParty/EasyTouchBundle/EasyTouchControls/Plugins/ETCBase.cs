...
public abstract class ETCBase : MonoBehaviour {
	...
	protected virtual void Awake(){
        ...
        {
            cachedCamera = cachedRootCanvas.worldCamera;
        }
    ...
    {
        cachedRootCanvas = transform.parent.GetComponentInParent<Canvas>();
        if (cachedRootCanvas != null)
        {
            cachedCamera = cachedRootCanvas.worldCamera;
        }
	...
	public virtual void LateUpdate(){
		//Logger.LogErrorFormat("1-1-1:{0}", Time.realtimeSinceStartup);
		if (enableCamera){
	...
	IEnumerator UpdateVirtualControl() {
		...
		//Logger.LogErrorFormat("1:{0}", Time.realtimeSinceStartup);
			DoActionBeforeEndOfFrame();

			yield return Yielders.EndOfFrame;

			//UpdateControlState();
		//Logger.LogErrorFormat("1-1:{0}", Time.realtimeSinceStartup);
	}
