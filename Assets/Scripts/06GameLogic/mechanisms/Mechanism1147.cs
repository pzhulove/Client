...
/// E: 瞄准移动速度(默认100)
/// </summary>
public class Mechanism1147 : LockAttackMechanism
{
    /// <summary>
    /// 内部表现类
    /// </summary>
    ...
    {
        public bool NewAnim = true;
        private SkillMonsterSniperFrame2 _sniperFrame;
        private SkillMonsterSniperOtherFrame2 _sniperOtherFrame; //其他人看到的狙击枪口页面
        ...
        public void InitConfig(int uiRadius, int sniperMoveSpeed, int endScale, int beginScale, float scaleSpeed)
        {
#if !LOGIC_SERVER
        ...
        /// 同步瞄准镜镜头
        /// </summary>
        public void UpdateSight(BeEntity target, int deltaTime = 0)
        {
#if !LOGIC_SERVER
            if (target != null)
            {
            ...
            if (_sniperFrame != null)
            {
                ...
                if (NewAnim)
                {
                    _sniperFrame.SniperScale(deltaTime);
                }
            }
#endif
        }
        ...
        private void InitFrame()
        {
            ...
            if (_topActor != null && _topActor.isLocalActor)
            {
                if (_sniperFrame == null)
                {
                    _sniperFrame = CreateFrame<SkillMonsterSniperFrame2>();
                }
            }
            else
            {
                ...
                if (BattleMain.IsModePvP(_owner.battleType))
                {
                    if (_sniperOtherFrame == null)
                    {
                        _sniperOtherFrame = CreateFrame<SkillMonsterSniperOtherFrame2>();
                    }
                }
            }
            InitSnipePosition();
        ...
        private SkillMonsterSniperFrame2 GetFrame()
        {
            if (_topActor != null && _topActor.isLocalActor)
            {
                return _sniperFrame;
        ...
        private T CreateFrame<T>() where T : SkillMonsterSniperFrame2
        {
    ...
    public Mechanism1147(int mid, int lv) : base(mid, lv) { }

    protected int m_checkTargetUIRadius = 270;
    private int m_SniperScale = 1500;
    ...
    private int[] mSniperStateTime = new int[(int)SightState.Max];
    private int curStateTime = 0;
    private SightState mCurSniperState = SightState.Start;
    ...
    public override void OnInit()
    {
        ...
        {
            m_SniperScale = TableManager.GetValueFromUnionCell(data.ValueB[0], level);
        }
    ...
    private void _InitGraphic()
    {
        ...
        _graphic.InitConfig(m_checkTargetUIRadius, m_sightMoveSpeed, 0, m_SniperScale,  (float)1000 / (float)m_AutoAttackTimeCD);
    }
    ...
    {
        mSniperStateTime[(int) SightState.Start] = m_delayMove;
        mSniperStateTime[(int) SightState.Move] = m_AutoAttackTimeCD;
        mSniperStateTime[(int) SightState.Pre] = 0;
        mSniperStateTime[(int) SightState.Post] = m_attackDelayMove;
    }
    ...
    private void UpdateStateTime(int deltaTime)
    {
        ...
        int curStateTimeOut = mSniperStateTime[(int) mCurSniperState];
        if (curStateTime >= curStateTimeOut)
        {
            mCurSniperState = ChangeToNextState(mCurSniperState);
            curStateTime = 0;
        }
    }

    private void UpdateStateAction(int deltaTime)
    {
        // 只有Move能移动
        if (mCurSniperState == SightState.Move)
        {
            _graphic.UpdateSight(_target, deltaTime);    
        }
        else
        {
    ...
    {
        SightState nextState = (curState + 1);
        if (nextState >= SightState.Max)
        {
            nextState = SightState.Move;
        }

        // 开枪前摇结束射击
        if (nextState == SightState.Post)
        {
            if (m_Skill != null) 
                m_Skill.DoAttack();
        }
