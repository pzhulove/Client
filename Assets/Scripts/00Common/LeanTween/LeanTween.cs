...
public class LeanTween : MonoSingleton<LeanTween>
{
    ...
    public GameObject     frameGlassMask;

    public AnimationCurve buttonTween;
    public Vector3        buttonPressScale;
    
    public float          cameraOrthoSize = 3.0f;

    public float          LevelUpNotifyDelay = 1.0f;
    public float          loadWaitTime = 0.0f;
    ...
    {
        LTDescr descr = value(frameroot,0.0f,1.0f,instance.frameOpenTime);
        descr.setOnComplete(OnCompleteaction);
        descr.setOnUpdate((scaleValue)=>{frameroot.transform.localScale = new Vector3(scaleValue,scaleValue,scaleValue);});
        descr.animationCurve = instance.frameOpenTween;

        if(blackMask)
        {
            var render = blackMask.GetComponent<CanvasRenderer>();
            if(render)
            {
                var alphaMask = value(blackMask,0.0f,1.0f,instance.frameOpenTime);
                alphaMask.setOnUpdate((alphaValue)=>{render.SetAlpha(alphaValue);});
            }
        }
        return descr;
    }
    ...
    {
        LTDescr descr = value(frameroot,0.0f,1.0f,instance.frameCloseTime);

        if(descr != null)
        {
            descr.setOnComplete(OnCompleteaction);
            descr.setOnUpdate((scaleValue) => { frameroot.transform.localScale = new Vector3(scaleValue, scaleValue, scaleValue); });
            descr.animationCurve = instance.frameCloseTween;
        }
        ...
        if(blackMask)
        {
            ...
            if(render)
            {
                var alphaMask = value(blackMask,1.0f,0.0f,instance.frameOpenTime);
                alphaMask.setOnUpdate((alphaValue)=>{render.SetAlpha(alphaValue);});
            }
        }
    ...
    {
        float value = tweenDescr.animationCurve.Evaluate(ratioPassed);
        return new Vector3(tweenDescr.from.x + (tweenDescr.diff.x *value), tweenDescr.from.y + (tweenDescr.diff.y * value), tweenDescr.from.z + (tweenDescr.diff.z * value));
    }
