...
{
    public const long kFactorDen  = 10000;
    public const long kIntDen     = 10000;
    ...
    //private static long[] kConvertFloatBit2Value = new long[] {
    //    /* 0  */ 1,
    //    /* 1  */ 10,
    //    /* 2  */ 100,
    //    /* 3  */ 1000,
    ...
    //private static long _convertFloatBit2Value(int bits)
    //{
    //    if (bits < 0 || bits >= kConvertFloatBit2Value.Length)
    //    {
    //        throw new Exception("out of bit range");
    //    }

    //    return kConvertFloatBit2Value[bits];
    //}

    //private static int _convertFloatValue2Bit(long value)
    //{
    //    for (int i = 0; i < kConvertFloatBit2Value.Length; ++i)
    //    {
    //        if (value == kConvertFloatBit2Value[i])
    //        {
    //            return i;
    //        }
    //    }

    //    throw new Exception("invalid value or out of range");
    //}

	public static int Float2IntWithFixed(float x, long validBit = kIntDen, long floatBit = kFloatBit, MidpointRounding rounding = MidpointRounding.ToEven)
    {
        return Float2IntWithFixed((double)x, validBit, floatBit, rounding);
    }

	public static int Float2IntWithFixed(double x, long validBit = kIntDen, long floatBit = kFloatBit, MidpointRounding rounding = MidpointRounding.ToEven)
	{
        //return (int)Math.Round(x * validBit, _convertFloatValue2Bit(floatBit), rounding);

		long roundInt 	= (long)Math.Round(x * (validBit * floatBit), rounding);
		bool isNegative = roundInt < 0;

		roundInt 	    = Math.Abs(roundInt);

		long validPart 	= roundInt / floatBit;
		long floatPart 	= roundInt % floatBit;

		if (floatPart >= (floatBit / 2))
		{
			validPart++;
		}

        return (int)(isNegative ? -validPart : validPart);
	}


    public static int Float2Int(float f,int d)
    {
         return Float2IntWithFixed(f * (float)d, GlobalLogic.VALUE_1, 10000);
         //return (int) Math.Round((double) (f * (float)d));
    }

    public static int Float2Int(float f)
    {
         return Float2IntWithFixed(f, GlobalLogic.VALUE_1, 10000);
         //return (int) Math.Round((double) (f));
    }

    public static int Double2Int(double d)
    {
         return Float2IntWithFixed(d, GlobalLogic.VALUE_1, 10000);
    ...
    public static VFactor acos(long nom, long den)
    {
        ...
        return new VFactor { nom = AcosLookupTable.table[num], den = kFactorDen };
    }
    ...
    public static VInt Mutiple(VInt a,VInt b)
    {
        long valuea = a.i;
        long valueb = b.i;
        long value = valuea * valueb / (long)IntMath.kIntDen;
        a.i = (int) value;
        return a;
    }
