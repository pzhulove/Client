...
public class ComHUDDebugger : MonoBehaviour
{
    ...
    private Rect _windowRect = new Rect(500, 0, 100, 60);
    private int _fps = 0;
    ...
    private GameObject _ActorRoot = null;
    ...
    private void ExpansionGUIWindow(int windowId)
    {
        ...
        {
            if (GUILayout.Button("PreLoadUI ON", GUILayout.Height(40)))
            {
        ...
        {
            if (GUILayout.Button("PreLoadUI OFF", GUILayout.Height(40)))
            {
        ...
        else if (_debugType == DebugType.Environment)
        {
            ...
            {
                if (null == _ActorRoot)
                    _ActorRoot = GameObject.Find("ActorRoot");

                if (null != _ActorRoot)
                    _ActorRoot.SetActive(!_ActorRoot.activeSelf);
            }
            if (GUILayout.Button("隐藏场景"))
            {
                if (null == _SceneRoot)
                    _SceneRoot = GameObject.Find("SceneRoot");

                if (null != _SceneRoot)
                    _SceneRoot.SetActive(!_SceneRoot.activeSelf);
            }
            GUILayout.EndHorizontal();
        }
        #endregion
