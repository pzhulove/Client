...
//[ExecuteInEditMode]
public class ComBattleExpBar : MonoBehaviour
{
    ...
    private void _updateText(UInt64 curExp, UInt64 curSum, ulong a_uTotalExp)
    {
        ...
        if (null != mExpText)
        {
            ...
            else
            {
                ...
                {
                    mExpText.text = string.Format("Exp:{0}/{1}", curExp, curSum, (curExp/ curSum) * 1.0f);
                }
            }
        }
    }
    ...
    private void _updateExp(UInt64 exp)
    {
        ...
        {
            _expImage.fillAmount = 1.0f * kv.Key / kv.Value;
