...
namespace GameClient
{
    ...
    /// 职业相关UI
    /// </summary>
    public class BattleUIProfession : BattleUIBase
    {
        public BattleUIProfession() : base() { }
        ...
        private ComCommonBind mJianqiBarBind = null;
        ...
        protected override void _bindExUI()
        {
            ...
            // 明王剑气
            mJianqiBarBind = mBind.GetCom<ComCommonBind>("JianqiBar");

            // 武斗化劲
            textMuscleShift = mBind.GetCom<Text>("textMuscleShift");
            iconMuscleShift = mBind.GetGameObject("iconMuscleShift");
            imageMuscleShift = mBind.GetCom<Image>("imageMuscleShift");
        }
        ...
        protected override void _unbindExUI()
        {
            ...
            textMuscleShift = null;
            iconMuscleShift = null;
            imageMuscleShift = null;
        }
        #endregion

        protected override string GetPrefabPath()
        {
            return "UIFlatten/Prefabs/BattleUI/BattleUIComponent/BattleUIProfession";
        }
        ...
        public void SetNvDaQiangEnergyBar2(int times)
        {
            if (null != pointerTransform && times >= 0 && times < angle.Length)
            {
                var mainQuence = DOTween.Sequence();
                ...
                if (energyProcess > times)
                {
                    var quence = DOTween.Sequence();
                    int length = endIndex - startIndex;
                    quence.Append(pointerTransform.transform.DORotate(new Vector3(0, 0, angle[times]), 0.166f));
        ...
        public void JianQiActive(bool active)
        {
            if (mJianqiBarBind != null) 
                mJianqiBarBind.gameObject.SetActive(active);
        }
        public void SetJianqi(int count, int max)
        {
            if (mJianqiBarBind != null)
            {
                float value = (float)count / (float)max;
                var slider = mJianqiBarBind.GetCom<Slider>("Slider");
                slider.value = value;
                
                var countText = mJianqiBarBind.GetCom<Text>("Count");
                countText.text = count.ToString();
            }
        }
        
        public void SetJianqiDown(int time, int max)
        {
            if (mJianqiBarBind != null)
            {
                ...
                var slider = mJianqiBarBind.GetCom<Slider>("Slider");
                slider.value = value;
                var count = mJianqiBarBind.GetCom<Text>("Count");
                count.text = (time / 1000).ToString();
