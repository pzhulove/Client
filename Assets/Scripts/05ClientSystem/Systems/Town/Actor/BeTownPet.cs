...
{
    public class BeTownPetData : BeBaseActorData
    {
        public int PetID { get; set; }
    }


    class BeTownPet : BeBaseActor
    {
        BeTownPlayer m_owner = null;
        ...
        protected Action<GeActorEx> m_OpenEggPostCallback;
        protected bool m_IsOpenEgg = false;

        protected IBeTownActionPlay _beTownActionPlay = null;
        public IBeTownActionPlay BeTownActionPlay { get { return _beTownActionPlay; }  }

        public BeTownPet(BeTownPetData data, ClientSystemTown systemTown,bool needDialog = false,bool isOpenEgg = false)
            : base(data, systemTown)
        {
            data.MoveData.MoveSpeed = Vector3.zero;
            m_bNeedDialog = needDialog;

            BeTownPetData townData = ActorData as BeTownPetData;
        ...
        /// 初始化开蛋角色创建
        /// </summary>
        public void InitPetForOpenEgg(GeSceneEx geScene, Action<GeActorEx> postLoad)
        {
            m_OpenEggPostCallback = postLoad;
            InitGeActor(geScene);
        }
        ...
        public override void InitGeActor(GeSceneEx geScene)
        {
            ...
            if (_geActor == null)
            {
                ...
                bool isTownMainPlayer = false;
                if (m_owner != null && (m_owner as BeTownPlayerMain) != null)
                    isTownMainPlayer = true;

                var resData = TableManager.GetInstance().GetTableItem<ProtoTable.ResTable>(m_petTable.ModeID);
                if (null != resData)
                {
                    // 如果启用了AvatarFallback，先检查资源是否存在，不存在不创建宠物资源(主玩家会创建）。
                    if (!isTownMainPlayer && (GeAvatarFallback.IsAvatarPartFallbackEnabled() && !GeAvatarFallback.IsAssetDependentAvaliable(resData.ModelPath)))
                    {
                        return;
                    }
                }

                //_geActor = geScene.CreateActor(m_petTable.ModeID, 0, 0, false, false);
                _geActor = geScene.CreateActorAsyncEx(m_petTable.ModeID, 0, 0, false, false,true, _PostLoad);
                if (_geActor == null)
                {
                    Logger.LogErrorFormat("InitGeActor CreateGeActor failed {0}", m_petTable.ModeID);
                }
                else
                {
                ...
                // 				entityPlane.w -= 0.07f;
                // 				GeSimpleShadowManager.instance.AddShadowObject(_geActor.renderObject, entityPlane,Vector3.one);
                //                 _InitGeDialog();
#endif
            }
        ...
        private void _PostLoad(GeActorEx pet)
        {
            ...
            if (m_petTable.UseNewFunction)
            {
                ...
                {
                    _beTownActionPlay = new BeTownPetActionPlay();
                    _beTownActionPlay.Init(_geActor, resTable.ActionConfigPath2);
                }
            }
            ...
            UpdateMove(0);
            if (m_OpenEggPostCallback != null)
                m_OpenEggPostCallback(pet);
#endif
        }
        ...
        public override void UpdateGeActor(float timeElapsed)
        {
            ...
            if (_beTownActionPlay != null)
            {
                _beTownActionPlay.Update(timeElapsed);
            }
        }

        public override void DoActionIdle()
        {
            if(_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.Idle)))
            {
                return;
            }
        ...
        {
            if (_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.HungerIdle)))
            {
                return;
            }
        ...
        {
            if (_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.SpecialIdle)))
            {
                return;
            }
        ...
        {
            if (_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.Walk)))
            {
                return;
            }
        ...
        {
            if (_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.HungerWalk)))
            {
                return;
            }
        ...
        void _InitGeDialog()
        {
            ...
            if (_geActor != null)
            {            
                m_objDialogRoot = CGameObjectPool.instance.GetGameObject("UIFlatten/Prefabs/Pet/PetDialog",enResourceType.UIPrefab,(uint)GameObjectPoolFlag.None);
                //GameObject objRoot = _geActor.GetEntityNode(GeEntity.GeEntityNodeType.Root);
                //Vector3 vecPos = m_objDialogRoot.transform.localPosition + _geActor.overheadLocalPosition;
                //Battle.GeUtility.AttachTo(m_objDialogRoot, objRoot);
                //m_objDialogRoot.transform.localPosition = vecPos;

                GameObject SceneUILayer = ClientSystemManager.GetInstance().SceneUILayer;
                if (SceneUILayer != null)
                {
                    m_rectDialogParent = SceneUILayer.GetComponent<RectTransform>();
                }
                ...
                Camera uiCam = ClientSystemManager.GetInstance().UICamera;
                if (uiCam != null)
                {
                    m_cameraUI = uiCam;
                }
        ...
        void _ShowDialog(int a_nDialogBaseID)
        {
            if (m_objDialogRoot != null && m_labDialog != null)
            {
                ...
                if(petDialogBase != null)
                {
                    if(petDialogBase.DialogIDs.Count > 0)
                    {
                        if (petDialogBase.FilterType == ProtoTable.PetDialogBaseTable.eFilterType.Invalid)
                        {
                            ...
                            if(speechTable != null)
                            {
                                ...
                                SetDialogLocalPosition();
                                m_objDialogRoot.CustomActive(true);
                            }
                            else
                            {
                                Logger.LogErrorFormat("speechTable == null in [_ShowDialog], a_nDialogBaseID = {0}, MonsterSpeech tablid = {1}", a_nDialogBaseID, petDialogBase.DialogIDs[0]);
                                m_objDialogRoot.CustomActive(false);
                            }
                        }
                        else if (petDialogBase.FilterType == ProtoTable.PetDialogBaseTable.eFilterType.Random)
                        {
                            ...
                            {
                                Logger.LogErrorFormat("speechTable == null in [_ShowDialog], a_nDialogBaseID = {0}, MonsterSpeech tablid = {1}", a_nDialogBaseID, petDialogBase.DialogIDs[nIndex]);
                                m_objDialogRoot.CustomActive(false);
                            }
                        }
                    }
                    ...
                    {
                        Logger.LogErrorFormat("petDialogBase.DialogIDs.Count == 0 in [_ShowDialog], a_nDialogBaseID = {0}", a_nDialogBaseID);
                        m_objDialogRoot.CustomActive(false);
                    }
                }
                else
                {
                    Logger.LogErrorFormat("petDialogBase == null in [_ShowDialog], a_nDialogBaseID = {0}", a_nDialogBaseID);
                    m_objDialogRoot.CustomActive(false);
                }
            }
        }
            ...
            else
            {
                ...
                if (m_fSpecialIdleRemainTime <= 0.0f)
                {
                    ...
                    if (null != _geActor)
                    {
                        ...
                        //GeAnimationManager geAnimationMgr = _geActor.GetAnimation();
                        //if (null != geAnimationMgr)
                        //{
                        //    GeAnimDesc clip = geAnimationMgr.GetAnimClipDesc("Anim_Idle02");
                        //
                        //    if (null != clip)
                        //    {
                        //        m_fSpecialIdleRemainTime = clip.timeLen;
                        //        m_bPlaySpecialIdle = true;
                        //    }
                        //}
                    }
        ...
        {
            if (_beTownActionPlay != null && _beTownActionPlay.PlayAction(BePetActionSwitchHelper.GetActionNameByType(PetActionNameType.LastAction)))
            {
                return;
            }
