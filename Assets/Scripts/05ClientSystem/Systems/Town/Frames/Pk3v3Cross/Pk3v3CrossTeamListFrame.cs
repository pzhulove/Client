...
namespace GameClient
{
    ...
    public class Pk3v3CrossTeamListFrame : ClientFrame
    {
        ...
        void OnJoinRoom(int iIndex)
        {
            ...
            if (roomList.rooms[iIndex].isLimitPlayerLevel > 0 && roomList.rooms[iIndex].limitPlayerLevel > PlayerBaseData.GetInstance().Level)
            {
                SystemNotifyManager.SysNotifyFloatingEffect("等级不足");
                return;
            }

            if (roomList.rooms[iIndex].isLimitPlayerSeasonLevel > 0 && roomList.rooms[iIndex].limitPlayerSeasonLevel > SeasonDataManager.GetInstance().seasonLevel)
            {
            ...
            {
                SystemNotifyManager.SysNotifyFloatingEffect("目标已进入比赛");
                return;
            }
            ...
            uint iRoomID = roomList.rooms[iIndex].id;
            ...
            if (frameMgr.IsFrameOpen<PkSeekWaiting>() || Pk3v3CrossDataManager.GetInstance().CheckRoomIsMatching())
            {
            ...
            Pk3v3CrossDataManager.SendJoinRoomReq(roomList.rooms[iIndex].id,RoomType.ROOM_TYPE_THREE_SCORE_WAR);
        }
        ...
        void UpdateRoomScrollListBind(ComUIListElementScript item)
        {
            ...
            Image headIcon = combind.GetCom<Image>("headIcon");
            Image seasonLV = combind.GetCom<Image>("seasonLV");
            Text seasonText = combind.GetCom<Text>("seasonText");

            Image MainSeasonLV = combind.GetCom<Image>("MainSeasonLV");
            Image SubSeasonLV = combind.GetCom<Image>("SubSeasonLV");

            id.text = roominfo.id.ToString();
            Lock.gameObject.CustomActive(roominfo.isPassword > 0);
            RoomName.text = roominfo.name;
            state.text = Pk3v3CrossDataManager.GetInstance().GetRoomState((RoomStatus)roominfo.roomStatus);

            if(roominfo.isLimitPlayerSeasonLevel == 0)
            {
                rank.text = "无";
            }
            else if (SeasonDataManager.GetInstance().seasonLevel >= roominfo.limitPlayerSeasonLevel)
            {
            ...
            {
                rank.text = string.Format("<color=#f0cd0dff>{0}</color>", SeasonDataManager.GetInstance().GetSimpleRankName((int)roominfo.limitPlayerSeasonLevel));
            }

            if(roominfo.isLimitPlayerLevel == 0)
            {
                lv.text = "无";
            }
            else if (PlayerBaseData.GetInstance().Level >= roominfo.limitPlayerLevel)
            {
            ...
            roomtype.text = Pk3v3CrossDataManager.GetInstance().GetRoomType((RoomType)roominfo.roomType);
            ...
            if (joinGray != null)
            {
                if ((RoomStatus)roominfo.roomStatus == RoomStatus.ROOM_STATUS_BATTLE || (RoomStatus)roominfo.roomStatus == RoomStatus.ROOM_STATUS_MATCH
                    || (RoomStatus)roominfo.roomStatus == RoomStatus.ROOM_STATUS_READY
                    || roominfo.playerSize >= roominfo.playerMaxSize 
                    || (roominfo.isLimitPlayerLevel != 0 && roominfo.limitPlayerLevel > PlayerBaseData.GetInstance().Level) 
                    || (roominfo.isLimitPlayerSeasonLevel != 0 && roominfo.limitPlayerSeasonLevel > SeasonDataManager.GetInstance().seasonLevel))
                {
            ...
            int nJob = (int)roominfo.ownerOccu;
            JobTable jobData = TableManager.GetInstance().GetTableItem<JobTable>((int)nJob);
            if (jobData != null)
            {
                ...
                if (resData != null)
                {
                    ...
                    //}
                    ETCImageLoader.LoadSprite(ref headIcon, resData.IconPath);
                }
            }

            int nSeasonLv = (int)roominfo.ownerSeasonLevel;
            string path = SeasonDataManager.GetInstance().GetMainSeasonLevelSmallIcon((int)nSeasonLv);
            ETCImageLoader.LoadSprite(ref seasonLV, path);

            int nSeasonLevel = (int)roominfo.ownerSeasonLevel;
            ETCImageLoader.LoadSprite(ref MainSeasonLV, SeasonDataManager.GetInstance().GetMainSeasonLevelSmallIcon(nSeasonLevel));
            ETCImageLoader.LoadSprite(ref SubSeasonLV, SeasonDataManager.GetInstance().GetSubSeasonLevelIcon(nSeasonLevel));

            seasonText.text = SeasonDataManager.GetInstance().GetRankName((int)roominfo.ownerSeasonLevel, true);
...
//             RoomSimpleInfo[] rooms1 = new RoomSimpleInfo[5];
//             for(int i = 0;i < rooms1.Length;i++)
//             {
//                 rooms1[i] = new RoomSimpleInfo();
//             }
//             roomList.rooms = rooms1;
//             roomList.startIndex = 0;
//             roomList.total = 5;
        ...
        void SendNormalRoomListReq()
        {
            ...
            req.roomType = (byte)RoomType.ROOM_TYPE_THREE_SCORE_WAR;
        ...
        {
            Pk3v3CrossDataManager.SendJoinRoomReq(0, RoomType.ROOM_TYPE_THREE_FREE);
        }
        private void _onBtRankRoomButtonClick()
        {
            ...
            Pk3v3CrossDataManager.SendJoinRoomReq(0, RoomType.ROOM_TYPE_THREE_SCORE_WAR);
        }
