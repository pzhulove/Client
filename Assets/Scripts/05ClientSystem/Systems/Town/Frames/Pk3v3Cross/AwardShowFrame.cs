...
{
    class AwardShowFrame : ClientFrame
    {
        public static void Open(int id)
        {
            ClientSystemManager.GetInstance().CloseFrame<AwardShowFrame>();
            ...
            if(data != null)
            {
                ClientSystemManager.GetInstance().OpenFrame<AwardShowFrame>(FrameLayer.Middle, data);
            }
        }
        ...
        protected override void _OnOpenFrame()
        {
            ...
            AwardRankItem rankItem = userData as AwardRankItem;
            if(rankItem != null)
            {
                m_kData = new MissionManager.MissionScoreData();
                for(int i = 0;i < rankItem.arrItems.Count;i++)
                {
                    AwardItemData data = new AwardItemData();
                    data.ID = rankItem.arrItems[i].TableID;
                    data.Num = rankItem.arrItems[i].Count;

                    m_kData.awards.Add(data);
                }
            }
        ...
        class AwardItemObject : CachedObject
        {
            ...
            public override void OnCreate(object[] param)
            {
                ...
                THIS = param[3] as AwardShowFrame;
