...
namespace GameClient
{
    ...
    class PocketJarFrame : ClientFrame
    {
        ...
        public static void OpenLinkFrame(string strParam)
        {
            try
            {
                ...
                if (paramArray.Length > 0)
                {
                    ...
                    if (eType > EPocketJarType.Invalid && eType < EPocketJarType.Count)
                    {
                        ClientSystemManager.GetInstance().OpenFrame<PocketJarFrame>(FrameLayer.Middle, eType);
                    }
                    else
                    {
                        ClientSystemManager.GetInstance().OpenFrame<PocketJarFrame>(FrameLayer.Middle);
        ...
        [UIControl("Content/Tabs/Viewport/Content/GoldTab")]
        Toggle m_togGoldJar;

        [UIControl("Content/Tabs/Viewport/Content/MagicTab")]
        Toggle m_togMagicJar;

        [UIControl("Content/Tabs/Viewport/Content/MagicTab_Lv55")]
        Toggle m_togMagic_Jar_Lv55;
        ...
        void BindUIEvent()
        {
            if (m_togGoldJar != null)
            {
                m_togGoldJar.onValueChanged.AddListener(OnToggleGoldClick);
            }
            if (m_togMagicJar != null)
            {
                m_togMagicJar.onValueChanged.AddListener(OnToggleMagicClick);
            }
            if (m_togMagic_Jar_Lv55 != null)
            {
                m_togMagic_Jar_Lv55.onValueChanged.AddListener(OnToggleMagicJarLev55Click);
            }
        }
        ...
        void UnBindUIEvent()
        {
            if (m_togGoldJar != null)
            {
                m_togGoldJar.onValueChanged.RemoveListener(OnToggleGoldClick);
            }
            if (m_togMagicJar != null)
            {
                m_togMagicJar.onValueChanged.RemoveListener(OnToggleMagicClick);
            }
            if (m_togMagic_Jar_Lv55 != null)
            {
                m_togMagic_Jar_Lv55.onValueChanged.RemoveListener(OnToggleMagicJarLev55Click);
            }
        }
        ...
        {
            mCurPocketJarType = EPocketJarType.Gold;

            if (value == true)
            {
                JarDataManager.GetInstance().RequestJarBuyRecord(m_currGoldJarData.nID);
                m_comMagicTicket.CustomActive(false);
            }
        }

        void OnToggleMagicClick(bool value)
        {
            mCurPocketJarType = EPocketJarType.Magic;

            _InitMagicJarUI();

            if (value)
            {
                JarDataManager.GetInstance().RequestJarBuyRecord(m_magicJarData.nID);
                m_comMagicTicket.CustomActive(true);
            }
        }

        void OnToggleMagicJarLev55Click(bool value)
        {
            mCurPocketJarType = EPocketJarType.Magic_Lv55;
            _InitMagicJarUI();
            if (value)
            {
                JarDataManager.GetInstance().RequestJarBuyRecord(m_magicJarData.nID);
        ...
        void _InitUI()
        {
            m_togMagic_Jar_Lv55.gameObject.CustomActive(Utility.IsUnLockFunc((int)FunctionUnLock.eFuncType.MagicJarLv55));
            ...
            if (userData != null)
            {
                ...
                if (mCurPocketJarType == EPocketJarType.Gold)
                {
                    m_togGoldJar.isOn = true;
                }
                else if (mCurPocketJarType == EPocketJarType.Magic)
                {
                    m_togMagicJar.isOn = true;
                }
                else if (mCurPocketJarType == EPocketJarType.Magic_Lv55)
                {
                    m_togMagic_Jar_Lv55.isOn = true;
                }
            }
            else
            {
                if (Utility.IsUnLockFunc((int)FunctionUnLock.eFuncType.MagicJarLv55))
                {
                    m_togMagic_Jar_Lv55.isOn = true;
                }
                else
                            ...
                            toggle.onValueChanged.AddListener(var =>
                            {
                                if (isToggleInited)
                                {
                                    if (var)
                                    {
                                        ...
                                        if (m_togGoldJar != null && m_togGoldJar.isOn == true && m_currGoldJarData != null)
                                        {
                                            JarDataManager.GetInstance().RequestJarBuyRecord(m_currGoldJarData.nID);
                                        }
