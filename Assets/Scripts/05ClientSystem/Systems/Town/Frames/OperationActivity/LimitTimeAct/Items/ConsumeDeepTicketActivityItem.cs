...
namespace GameClient
{
    public class ConsumeDeepTicketActivityItem : ActivityItemBase
    {
        ...
        public override void Dispose()
        {
            ...
            mButtonGoActivity.onClick.RemoveAllListeners();
        }
        ...
        protected override void OnInit(ILimitTimeActivityTaskDataModel data)
        {
            ...
            mButtonGoActivity.onClick.AddListener(() =>
            {
            ...
            {
                if(data.ParamNums[0] == 0 && data.ParamNums2[0] == 0)
                {
                    //角色每日不刷新
                    mTextExchangeCount.text = TR.Value("activity_role_day_not_update_tips");
                }
                if (data.ParamNums[0] == 0 && data.ParamNums2[0] == 1)
                {
                    //角色每日刷新
                    mTextExchangeCount.text = TR.Value("activity_role_day_update_tips");
                }
                if (data.ParamNums[0] == 1 && data.ParamNums2[0] == 0)
                {
                    //账号每日不刷新
                    mTextExchangeCount.text = TR.Value("activity_account_day_not_update_tips");
                }
                if (data.ParamNums[0] == 1 && data.ParamNums2[0] == 1)
                {
                    //账号每日刷新
                    mTextExchangeCount.text = TR.Value("activity_account_day_update_tips");
                }
            }
        }
