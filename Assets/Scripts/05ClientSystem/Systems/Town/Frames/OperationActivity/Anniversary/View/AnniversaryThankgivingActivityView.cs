...
namespace GameClient
{
    public class AnniversaryThankgivingActivityView : MonoBehaviour, IGiftPackActivityView
    {
        ...
        private void _OnGetGiftData(UIEvent param)
        {
            ...
            if (mItemParent.Count<data.gifts.Length&&mNameTxt.Count<data.gifts.Length)
            {
                    ...
                    if (giftTable.ItemID > 0)
                    {
                        ...
                        if (giftTable.ItemID > 0&& comItem!=null)
                        {
                                                    ...
                                                    itemData.SubType == (int)ItemTable.eSubType.PetEgg)
                            {
                                ...
                                preViewItem.activityId = (int)mModle.Id;
                                preViewItem.itemId = itemData.TableID;
                                preViewDataModel.preViewItemList.Add(preViewItem);

                            }
                            mNameTxt[i].SafeSetText(itemData.Name);
                        }
        ...
        private void OnSyncWorldMallBuySucceed(UIEvent uiEvent)
        {
            ...
            if (itemId == mModle.DetailDatas[0].Id)
            {
                ...
                UpdateBuyButton();
                ActivityDataManager.GetInstance().RequestMallGiftData(MallTypeTable.eMallType.SN_GRATITUDE_GIFT);
            }
