...
{
    class EquipmentActivationView : StrengthGrowthBaseView
    {
        [SerializeField]private GameObject mItemParent;
        [SerializeField]private Button mActivationBtn;
        [SerializeField]private StateController mEquipStateCtrl;
        [SerializeField] private ComUIListScript mExpendUIList;
        [SerializeField]private List<Text> mAttrTexts;
        [SerializeField]private List<string> mAttrDescs;
        ...
        private void Awake()
        {
            ...
            if (mActivationBtn != null)
            {
                mActivationBtn.onClick.RemoveAllListeners();
                mActivationBtn.onClick.AddListener(OnActivationClick);
            }
        }
        ...
        private void RegisterUIEventHandle()
        {
            ...
            UIEventSystem.GetInstance().RegisterEventHandler(EUIEventID.OnBreathEquipActivationSuccess, OnBreathEquipActivationSuccess);
        ...
        private void UnRegisterUIEventHandle()
        {
            ...
            UIEventSystem.GetInstance().UnRegisterEventHandler(EUIEventID.OnBreathEquipActivationSuccess, OnBreathEquipActivationSuccess);
        ...
        private void OnStrengthenGrowthEquipItemClick(ItemData itemData, StrengthenGrowthType eStrengthenGrowthType)
        {
            ...
            if (eStrengthenGrowthType == mStrengthenGrowthType)
            {
                ...
                mExpendItemData = null;
                mAttrValue = EquipGrowthDataManager.GetInstance().GetGrowthAttributeValue(itemData, 0);
        ...
        private void FindBindExpendItemData(List<ulong> items, ref ItemData expendItemData)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (data.SubType != (int)ItemTable.eSubType.ST_ZENGFU_ACTIVATE)
                    continue;
        ...
        {
            if (mAttrTexts.Count == mAttrDescs.Count )
            {
                for (int i = 0; i < mAttrTexts.Count; i++)
                {
                    mAttrTexts[i].text = string.Format(mAttrDescs[i], mAttrValue);
                }
            }
        }
        ...
        private void OnActivationClick()
        {
            ...
            {
                SystemNotifyManager.SysNotifyFloatingEffect("已加锁的装备无法激活");
                return;
            }
            ...
            if (mExpendItemData == null)
            {
                ItemComeLink.OnLink(300000205, 0);
                return;
            }
            ...
            EquipGrowthDataManager.GetInstance().OnSceneEquipEnhanceRed(mSelectEquipItemData, (UInt32)mExpendItemData.TableID);
        }
    }
}