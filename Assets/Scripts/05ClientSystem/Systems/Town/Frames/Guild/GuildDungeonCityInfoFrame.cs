...
#region ExtraUIBind
        ...
        private BossGuildDungeon BOSS = null;
        private MediumGuildDungeon Medium0 = null;
        private JuniorGuildDungeon Junior0 = null;
        ...
        private void UpdateGuildDungeonInfo()
        {
            ...
            if (data != null)
            {
                GuildDungeonLvl lv = (GuildDungeonLvl)GuildDataManager.GetInstance().GetGuildDungeonDiffLevel((uint)mDungeonID.dungeonID);
                if (lv == GuildDungeonLvl.GUILD_DUNGEON_HIGUH)
                {
                    if (data.bossDungeonDamageInfos.Count > 0 && BOSS != null)
                    {
                        BOSS.SetUp(data.bossDungeonDamageInfos[0]);
                        BOSS.CustomActive(true);
                    }
                }
                else if (lv == GuildDungeonLvl.GUILD_DUNGEON_MID)
                {
                    for (int i = 0; i < data.mediumDungeonDamgeInfos.Count; i++)
                    {
                        if (data.mediumDungeonDamgeInfos[i].nDungeonID == (ulong)mDungeonID.dungeonID)
                        {
                            if (Medium0 != null)
                            {
                                Medium0.SetUp(data.mediumDungeonDamgeInfos[i]);
                                Medium0.CustomActive(true);
                                break;
                            }
                        }
                    }
                }
                else if (lv == GuildDungeonLvl.GUILD_DUNGEON_LOW)
                {
                    for (int i = 0; i < data.juniorDungeonDamgeInfos.Count; i++)
                    {
                        if (data.juniorDungeonDamgeInfos[i].nDungeonID == (ulong)mDungeonID.dungeonID)
                        {
                            if (Junior0 != null)
                            {
                                Junior0.SetUp(data.juniorDungeonDamgeInfos[i]);
                                Junior0.CustomActive(true);
                                break;
                            }
                        }
                    }
        ...
        private void _updateDungeonInfo()
        {
            if (null != mDungeonTable)
            {
                ...
                mDungeonUnitInfo.CustomActive(false);
            }
        }
        ...
        {           
            return !ChapterUtility.IsTeamDungeonID(mDungeonID.dungeonID) || _isTeamBattleLevelLimte();
        }
...
//             }
            ...
            if (TeamDataManager.GetInstance().HasTeam())
            {
                ...
                if (TeamDataManager.GetInstance().IsTeamLeader())
                {
                    ...
                    if (GuildDataManager.GetInstance().IsGuildDungeonMap(mDungeonID.dungeonID))
                    {
            ...
            else
            {
                ...
                if (GuildDataManager.GetInstance().GetGuildDungeonDiffLevel((uint)mDungeonID.dungeonID) == (int)GuildDungeonLvl.GUILD_DUNGEON_HIGUH)
                {
                    SystemNotifyManager.SysNotifyMsgBoxCancelOk(TR.Value("enterGuildBossDungeonJoinATeam"), null, () =>
                    {
                        EnterGuildDungeon();
                    });
                    return;
