...
namespace GameClient
{
    public class GuildDungeonAwardShowFrame : ClientFrame
    {
        ...
        GameObject effect_qian_go = null;
        GameObject effect_zi_go = null;
        ...
        void InitEffectRoot()
        {
            ...
            if (effect_qian_go == null)
            {
                effect_qian_go = AssetLoader.GetInstance().LoadResAsGameObject(EffUI_shouchong_qian_Path);
                Utility.AttachTo(effect_qian_go, mEffectRoot_Envior);
            }
            if (effect_zi_go == null)
            {
                effect_zi_go = AssetLoader.GetInstance().LoadResAsGameObject(EffUI_shouchong_zi_Path);
                Utility.AttachTo(effect_zi_go, mEffectRoot_TopTitle);
            }
            //if (effect_left_jiantou_go == null)
            //{
            //    effect_left_jiantou_go = AssetLoader.GetInstance().LoadResAsGameObject(EffUI_shouchong_jiantou_Path);
            //    Utility.AttachTo(effect_left_jiantou_go, mEffectRoot_LeftBtn);
            //}
            //if (effect_right_jiantou_go == null)
            //{
            //    effect_right_jiantou_go = AssetLoader.GetInstance().LoadResAsGameObject(EffUI_shouchong_jiantou_Path);
        ...
        void ClearEffectRoot()
        {
            ...
            if (effect_qian_go)
            {
                GameObject.Destroy(effect_qian_go);
                effect_qian_go = null;
            }
            if (effect_zi_go)
            {
                GameObject.Destroy(effect_zi_go);
                effect_zi_go = null;
            }
        ...
        private void UpdatePayRewardItems(int selectPayActId)
        {
            ...
            awardItemDataList2 = GetKillBossAwardList();

            //mScrollUIList.m_autoCenteredElements = true;
            mScrollUIList2.SetElementAmount(awardItemDataList2.Count);

            //mScrollUIList.ResetContentPosition();
            //var scorllRect = mScrollUIList.gameObject.GetComponent<RectTransform>();
            //if (scorllRect)
            //{
            //    mScrollPadding = scorllRect.sizeDelta.x;
            //}
            //mScrollPadding = _CalAwardScorllListPadding(mScorllWidth, mScorllItemWidth, mScrollItemSpace, mScrollUIList.GetElementAmount());
            //mScrollUIList.m_elementPadding = new Vector2(mScrollPadding, 0f);
        ...
        {
            float scorllListPadding = 0f;
            scorllListPadding = (listWidth - (itemWidth * itemNum + (itemNum - 1) * itemSpace)) * 0.5f;
            return scorllListPadding;
        }
        ...
        private void ClearData(bool isClearWithTabs = true)
        {
            ...
            if (mTabScrollUIList != null)
            {
                mTabScrollUIList.SetElementAmount(0);
            }
        }
        ...
        {
            if (PayManager.GetInstance().HasConsumePay())
            {
                ClearData();
                bGotReward = true;
                UpdateFrame();
...
//         [UIEventHandle("btnClose")]
		...
		private GameObject mEffectRoot_TopTitle = null;
		...
		protected override void _bindExUI()
		{
			...
			mEffectRoot_TopTitle = mBind.GetGameObject("EffectRoot_TopTitle");
