...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  -->public partial class <xsl:call-template name="pascal"/> : global::ProtoBuf.IExtensible,global::ProtoBuf.IParseable, ITable<!--
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_REPEATED']">
    ...
    <xsl:variable name="field"><xsl:apply-templates select="." mode="field"/></xsl:variable>
    [UnityEngine.SerializeField]
    private <xsl:if test="not($optionXml)"></xsl:if> global::System.Collections.Generic.List&lt;<xsl:value-of select="$type" />&gt; <xsl:value-of select="$field"/> = new global::System.Collections.Generic.List&lt;<xsl:value-of select="$type"/>&gt;();
