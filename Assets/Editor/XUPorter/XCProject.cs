	...
	public partial class XCProject : System.IDisposable
	{
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false )
		{
			...
			if( !( File.Exists( absPath ) || Directory.Exists( absPath ) ) && tree.CompareTo( "SDKROOT" ) != 0 ) {
                ...
                Debug.LogFormat("[debug] relativeUri abspath {0}, filepath {1}", absPath, filePath);
			}
            ...
            Debug.LogFormat("[debug] before path {0}", filePath);

			fileReference = new PBXFileReference( filePath, (TreeEnum)System.Enum.Parse( typeof(TreeEnum), tree ) );
			parent.AddChild( fileReference );
			fileReferences.Add( fileReference );

            Debug.LogFormat("[debug]final name {0}, path {1}", fileReference.name, fileReference.path);
		...
		{	
            ...
            {
                Debug.Log( "[debug]modepath : " + mod.path);
                Debug.Log( "[debug]filepath : " + filePath);

				string absoluteFilePath = System.IO.Path.Combine( mod.path, filePath );

                Debug.Log( "[debug]Adding file : " + absoluteFilePath);
