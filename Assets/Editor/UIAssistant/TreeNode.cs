...
public class TreeNode
{
    ...
    private void GenInfo() 
    {
        ...
        var image = assetObject.GetComponent<Image>();
        var imageEx = assetObject.GetComponent<ImageEx>();
        var imageNull = assetObject.GetComponent<NullImage>();
        var text = assetObject.GetComponent<Text>();
        bool flag = true;
        if (image != null)
        {
            if(!image.enabled || image.fillAmount == 0)
                flag = false;
        }
        if (imageEx != null)
        {
        ...
        var rectTransforms = assetObject.GetComponentsInParent<RectTransform>();
        foreach (var item in rectTransforms)
        {
            if (item.localScale.x == 0 && item.localScale.y == 0)
            {
                flag = false;
                break;
            }
        ...
        var canvasGroups = assetObject.GetComponentsInParent<CanvasGroup>();
        ...
        var canvasGroupSelf = assetObject.GetComponent<CanvasGroup>();
        if (canvasGroupSelf != null)
        {
            if (canvasGroupSelf.alpha == 0)
            {
                flag = false;
            }
        }
        /*var graphic = assetObject.GetComponent<Graphic>();
        if (graphic != null)
        {
            ...
            if (fieldInfo != null)
            {
                ...
                if (retObj != null)
                {
                    ...
                    if(retObj.currentIndexCount <= 0)
                        flag = false;
                }
            }
        }*/
        
        nodeInfo = new UINodeInfo(ui != null && flag, assetObject.GetComponent<Canvas>() != null, assetObject.name);
