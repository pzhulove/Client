...
public class XlsxDataEditorWindow : EditorWindow
{
    ...
    //[MenuItem("转表工具/PB_CS添加宏")]
    public static void AddPBMacro()
    {
        string folderPath = "Assets\\Scripts\\01TableScript\\table";
        DirectoryInfo dirOutPath = new DirectoryInfo(folderPath);
        ...
        foreach (FileInfo objFile in objFileList)
        {
            ...
            if (!fileName.StartsWith(".") && !fileName.Contains(".meta") && fileName.Contains(".cs"))
            {
                ...
                byte[] array = Encoding.UTF8.GetBytes("#if !USE_FB_TABLE\n");
                outFs.Write(array, 0, array.Length);
                outFs.Write(heByte, 0, heByte.Length);
    ...
    {
        editorWindow = EditorWindow.GetWindow<XlsxDataEditorWindow>(false, "PB转表", true);
        editorWindow.Show();
    ...
    private void Refresh()
    {
        ...
        for (int i = 0; i < m_pFileList.Length; i++)
        {
            ...
            string hash = DataUnitPath.GetMd5Hash(path);
            if (hash == m_pXlsMD5[i])
            {
                m_pToggle[i] = false;
            }
    ...
    public void OnGUI()
    {
        ...
        if (mBuildProto)
        {
            ...
            for (int i = 0; i < m_pFileList.Length; i++)
            {
                ...
                if (m_pToggle[i])
                {
                    ...
                    m_pXlsMD5[i] = DataUnitPath.GetMd5Hash(path);
                }
            }
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            EditorGUILayout.BeginVertical();
            {
                ...
                //if (XlsxDataManager.StyledButton("转下Union.cs!"))
                //{
                //    string protoPath = DataUnitPath.CombinePath("Union", DataUnitType.PROTO);//.PROTO_PATH, m_szProtoName);
                //    string codePath  = DataUnitPath.CombinePath("Union", DataUnitType.CSCODE);
                //    string codecPath = DataUnitPath.CombinePath("Union", DataUnitType.CCODE);
