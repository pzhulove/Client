...
namespace GenerateCode
{
    public class EnumGeneratorWindow : EditorWindow
    {
                ...
                switch (useValues)
                {
                    ...
                    case UseValues.UnityLayers:
                        ...
                        for (int i = 0; i < layers.Length; i++)
                        {
                            ...
                            int index = withFlagsAttribute ? (1 << layerValue) : layerValue;
            ...
            if (GUILayout.Button("Save", GUILayout.MaxWidth(80f)))
            {
                ...
                switch (useValues)
                {
                    ...
                    case UseValues.UnityLayers:
                        ...
                        enumNums   = new int[enumValues.Length];
                        for(int i = 0; i < enumValues.Length; ++i)
                        {
                            enumNums[i] = LayerMask.NameToLayer(enumValues[i]);
                        }
                        flag = true;
                        break;
                    case UseValues.UnityTags:
                ...
                // write .cs file
                Write(codeNamespace, codeEnumName, withFlagsAttribute, enumValues, enumNums,flag,folder);
                Close();
            }
        ...
        public static void Write(string ns, string typeName, bool withFlagsAttribute, string[] fields, int[] values,bool flag,string folder = null)
        {
            // get full path
            ...
            {
                if(flag)
                    EnumGenerator.WriteEnumEx(writer,ns,typeName,fields,values,withFlagsAttribute,null,null);
                else
