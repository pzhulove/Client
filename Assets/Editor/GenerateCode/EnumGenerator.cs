...
namespace GenerateCode
{
    public static class EnumGenerator
    {
        ...
        public static void WriteEnumEx(TextWriter writer, string ns, string typeName, string[] fields, int[] values,bool withFlagsAttribute = false, string zerothField = null, string allField = null)
        {
            ...
            string prifex = "Layer_";
            // creating enum fields
            foreach (var valueName in fields)
            {
                // create enum fields
                var codeField = new CodeMemberField(codeType.Name,  prifex + valueName.Replace(' ','_'));

                if (withFlagsAttribute)
                {
                    // flags have values that are a power of 2 (each its own bit 1, 2, 4, 8, 16), 
                    codeField.InitExpression = new CodePrimitiveExpression(1 << values[index++]);// index is the index of the bit
                }
                else
                {
                    // just the default zero-based index (0, 1, 2, 4, 5)
                    codeField.InitExpression = new CodePrimitiveExpression(values[index++]);
                }
                // add field to type
                codeType.Members.Add(codeField);
            }
            ...
            {
                var codeField = new CodeMemberField(codeType.Name, allField);
                codeField.InitExpression = new CodePrimitiveExpression((1 << values[index])-1);
                codeType.Members.Add(codeField);
            }
